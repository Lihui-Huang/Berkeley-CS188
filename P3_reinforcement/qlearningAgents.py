...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.qvalues[(state, action)] = self.qvalues[(state, action)] * (1 - self.alpha) + self.alpha * sample
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature in self.featExtractor.getFeatures(state, action):
            weight += self.weights[feature] * self.featExtractor.getFeatures(state, action)[feature]
        return weight
    ...
    def update(self, state, action, nextState, reward):
        ...
        for nextaction in nextactions:
            maxq = max(maxq, self.getQValue(nextState, nextaction))
        if maxq == float("-inf"):
            maxq = 0.0
            """
